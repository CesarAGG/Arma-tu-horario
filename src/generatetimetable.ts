...
export abstract class timeTableDrawer {
    ...
    private static generateScheduleMatrix(schedule: Schedule) {
        let matrix: boolean[][] = [];
        for (let day of DAYS) {
    ...
    static drawSchedule(schedule: Schedule): void {
        window.setTimeout(function () {
            var element = document.getElementById("table");
            if (element) {
                timeTableDrawer._drawSchedule(schedule);
            } else {
                timeTableDrawer.drawSchedule(schedule);
            }
        }, 500)
    }
